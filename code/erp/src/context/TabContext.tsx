...
export const TabProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const openTab = useCallback((id: string, title: string, component: ReactNode | string, isClosable: boolean = true) => {
    setTabs(prevTabs => {
  ...
  const closeTab = useCallback((id: string) => {
    setTabs(prevTabs => {
      ...
      if (filteredTabs.length > 0) {
        ...
        setActiveTabIdState(currentActive => currentActive === id ? filteredTabs[filteredTabs.length - 1].id : currentActive);
      } else {
