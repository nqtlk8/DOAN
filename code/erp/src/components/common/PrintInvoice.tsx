...
export const PrintInvoice: React.FC<PrintInvoiceProps> = ({
  ...
  const displayTitle = title || (mode === 'quote' ? 'BÁO GIÁ / ĐẶT TRƯỚC' : mode === 'return' ? 'PHIẾU KHÁCH TRẢ HÀNG' : mode === 'purchase' ? 'PHIẾU NHẬP HÀNG' : 'PHIẾU XUẤT HÀNG');
  const now = new Date();
  ...
  return (
    <div className={`p-8 bg-white text-black font-sans text-[15px] ${className}`}>
      ...
      <div className="text-center mb-6">
        ...
        <p className="italic text-sm">ngày {now.getDate()} tháng {now.getMonth() + 1} năm {now.getFullYear()}</p>
      </div>
        ...
        <tbody>
          {items.map((item, index) => (
            <tr key={index} className="border-b border-slate-300">
              ...
              {!hidePrices && <td className="py-1 border-r border-black text-right px-2">{formatMoney(item.unitPrice)}</td>}
              {!hidePrices && <td className="py-1 text-right px-2">{formatMoney(item.quantity * item.unitPrice)}</td>}
            </tr>
          ))}
          {items.length === 0 && (
            <tr>
              <td colSpan={hidePrices ? 3 : 5} className="py-4 text-center text-slate-500 italic">Không có sản phẩm</td>
            </tr>
          )}
        </tbody>
      </table>
                ...
                {mode !== 'return' && mode !== 'quote' && (
                  <>
                    ...
                    <tr>
                      ...
                      <td className="py-1 pt-2 text-right">{formatMoney(remainingBalance + oldDebt)}</td>
                    </tr>
                  </>
                )}
              </tbody>
