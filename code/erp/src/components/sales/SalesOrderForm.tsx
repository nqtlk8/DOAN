...
export const SalesOrderForm = forwardRef<SalesOrderFormRef, SalesOrderFormProps>(({ mode: initialMode = 'VIEW', initialData, onStateChange }, ref) => {
  const { user } = useAuth();
  ...
  return (
    <div className="flex flex-col h-full bg-white relative">
      ...
      <GenericDocumentForm
        mode={mode}
        title={mode === 'ADD' ? 'Thêm mới Đơn hàng' : mode === 'EDIT' ? `Sửa Đơn: ${orderCode}` : `Đơn Bán Hàng: ${orderCode}`}
        error={error}
        orderCode={orderCode}
        creator={creator}
        branch={branch}
      ...
      <SearchModal<Customer>
        ...
        fetchData={(query) => CatalogService.searchCustomers(query)}
        renderItem={(c) => (
          <div>
