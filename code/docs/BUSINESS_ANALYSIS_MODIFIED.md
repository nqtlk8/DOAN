...
> - Mở rộng mô hình tồn kho từ 2 lớp lên 3 lớp: `stock_on_hand` + `cost_layer` + `stock_movement`
>
> **Changelog v1.1:**
...
[DRAFT]  ─────── Xác nhận ──────→ [CONFIRMED]
   ...
   |  Gắn PO (tùy chọn)                 |  Tạo cost_layer mới (product + branch + cost + qty)
...
```
Nhập lô 1: 100 viên × 10,000đ  (layer_1)
Nhập lô 2:  50 viên × 15,000đ  (layer_2)

Bán đơn 1: 80 viên × 20,000đ
  → FIFO lấy từ layer_1: cost = 10,000đ
  → gross_profit_đơn_1 = (20,000 - 10,000) × 80 = 800,000đ

Bán đơn 2: 40 viên × 22,000đ
  → FIFO: 20 viên còn lại từ layer_1 (cost=10,000) + 20 viên từ layer_2 (cost=15,000)
...
Tạo cost_layer mới:
  ...
  created_at  = NOW()           ← dùng để sắp xếp FIFO
...
Bước 2 — Cập nhật lot:
  UPDATE cost_layer SET remaining_qty = 0   WHERE id = layer_1
  UPDATE cost_layer SET remaining_qty = 150 WHERE id = layer_2
...
Bước 4 — Ghi stock_movement (2 dòng vì lấy từ 2 lot):
  Dòng 1: movement_type='SALE', quantity=-100, unit_cost=10,000, inbound_movement_id=layer_1
  Dòng 2: movement_type='SALE', quantity=-50,  unit_cost=12,000, inbound_movement_id=layer_2
...
→ gross_profit của sales_invoice_line này:
  ...
  = Tính từ stock_movement, không phải từ 1 con số bình quân
```
...
Bán đơn 1: 80 viên × 20,000đ → lấy từ layer_1
  stock_movement: qty=-80, cost=10,000
  gross_profit = (20,000-10,000)×80 = 800,000đ

Bán đơn 2: 40 viên × 22,000đ → 20 từ layer_1, 20 từ layer_2
  stock_movement dòng 1: qty=-20, cost=10,000
  stock_movement dòng 2: qty=-20, cost=12,000
...
| Thay đổi | Chi tiết |
|---|---|
| **THÊM** bảng `cost_layer` | `product_id, branch_id, unit_cost, initial_qty, remaining_qty, created_at, inbound_movement_id` |
| **THÊM** bảng `stock_movement` | `product_id, branch_id, movement_type, quantity, unit_cost, balance_qty, balance_value, ref_type, ref_id, ref_line_id, performed_by, created_at` |
...
Trong 1 transaction xác nhận hóa đơn bán hàng (CONFIRMED):
  ...
  BEGIN;
    SELECT cost_layer WHERE remaining_qty > 0 ORDER BY created_at ASC FOR UPDATE
    UPDATE cost_layer SET remaining_qty = ...             (FIFO consumption)
    INSERT INTO stock_movement (...) VALUES (...)          (ghi lịch sử biến động — 1 dòng/lot tiêu thụ)
...
> Dùng **pessimistic lock** (`SELECT FOR UPDATE`) cho cost_layer vì FIFO đòi hỏi thứ tự chính xác, không thể retry nếu lot bị thay đổi giữa chừng.
...
| `stock_movement.balance_qty / balance_value` | Running balance per movement — đối soát nhanh không cần replay toàn bộ ledger |
...
| 3 | Race condition khi nhiều người bán cùng lúc | Optimistic lock `stock_on_hand.version` + Pessimistic lock `cost_layer` (FOR UPDATE) | JPA `@Version`, retry on `OptimisticLockException` |
...
| 9 | Lợi nhuận phản ánh đúng từng lô nhập | FIFO: tiêu thụ lot cũ nhất trước, snapshot cost per lot | Bảng `cost_layer`, `FOR UPDATE`, `unit_cost_snapshot` |
| 10 | Biết giao dịch nào tạo nên tồn kho hiện tại | Stock Movement: ghi mỗi biến động kho thành 1 dòng, có running balance | Bảng `stock_movement`, append-only, ref về chứng từ gốc |
