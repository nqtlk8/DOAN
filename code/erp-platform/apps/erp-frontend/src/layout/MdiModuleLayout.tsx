        ...
        <div className="flex flex-col p-2 space-y-1">
          <button
            ...
            className={`px-4 py-2 text-left rounded-md text-sm font-medium transition-colors ${
              activeSubView === 'FORM' ? 'bg-teal-50 text-teal-700' : 'text-slate-600 hover:bg-slate-50'
            }`}
          >
            Nội dung
          </button>
          <button
            ...
            className={`px-4 py-2 text-left rounded-md text-sm font-medium transition-colors ${
              activeSubView === 'LIST' ? 'bg-teal-50 text-teal-700' : 'text-slate-600 hover:bg-slate-50'
            }`}
          >
            Danh sách phiếu
          </button>
          ...
          <div className="bg-slate-800 text-white shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] px-4 py-2 flex items-center justify-between z-40 shrink-0 h-14">
            ...
            <div className="flex gap-2">
              {isView && (
                <>
                  <button
                    onClick={onAdd}
                    className="flex items-center gap-2 px-3 py-1.5 bg-slate-700 hover:bg-slate-600 rounded-md text-sm font-medium transition-colors"
                  >
                  ...
                  <button
                    onClick={onDelete}
                    className="flex items-center gap-2 px-3 py-1.5 bg-slate-700 hover:bg-red-600 hover:text-white rounded-md text-sm font-medium transition-colors text-red-400"
                  >
              ...
              {!isView && (
                <>
                  <button
                    onClick={onSave}
                    disabled={isLoading}
                    className="flex items-center gap-2 px-4 py-1.5 bg-teal-600 hover:bg-teal-500 rounded-md text-sm font-medium transition-colors disabled:opacity-50"
                  >
                    <Save size={16} /> Lưu (F4)
                  </button>
                  <button
                    onClick={onCancel}
                    disabled={isLoading}
                    className="flex items-center gap-2 px-3 py-1.5 bg-slate-700 hover:bg-slate-600 rounded-md text-sm font-medium transition-colors disabled:opacity-50"
                  >
                    <X size={16} /> Hủy (Esc)
                  </button>
                </>
            ...
            <div className="flex gap-2 border-l border-slate-600 pl-4">
              <button
                onClick={onPrint}
                disabled={!isView}
                className="flex items-center gap-2 px-3 py-1.5 bg-slate-700 hover:bg-slate-600 rounded-md text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Printer size={16} /> In (F7)
              </button>
              <button
                onClick={onExit}
                className="flex items-center gap-2 px-3 py-1.5 bg-slate-700 hover:bg-red-600 rounded-md text-sm font-medium transition-colors"
              >
